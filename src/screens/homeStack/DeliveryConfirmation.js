...
const DeliveryConfirmation = ({route, navigation}) => {
  ...
  const Item = ({value, id}) => (
    <TouchableOpacity
      ...
      onPress={() => onOrderRefused(value)}>
      <Text style={styles.listText}>{value}</Text>
    </TouchableOpacity>
  );

  const renderItem = ({item}) => <Item value={item.key} id={item.id} />;
  const renderInner = () => (
          ...
          <View style={styles.one}>
            ...
            <View style={styles.two}>
              ...
              {List ? (
                <FlatList
                  data={[
                    {key: 'Not available', id: 1},
                    {key: 'Fund not available', id: 2},
                    {key: 'Denied', id: 3},
                  ]}
