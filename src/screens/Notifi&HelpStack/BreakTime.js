...
const BreakTime = ({navigation}) => {
  ...
  const start = () => {
    ...
    startBreakTime(data.current_time_slot_id, data.driver_break_row_id).then(
      res => {
        if (res.code == 200) {
          if (res.success == 'false') {
            alert(res.message);
  ...
  const end = () => {
    ...
    endBreakTime(data.current_time_slot_id, data.driver_break_row_id).then(
      res => {
        if (res.code == 200) {
          if (res.success == 'false') {
            alert(res.message);
